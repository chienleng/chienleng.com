...
export default defineConfig({
	server: {
		// chienleng.localhost, not localhost: cookies are port-blind, so every
		...
		// cookie origin. The port is pinned (7606 in this machine's per-project
		// block) so bookmarks never chase an auto-incremented port.
		host: 'chienleng.localhost',
		port: 7606,
		strictPort: true
	},
	plugins: [
